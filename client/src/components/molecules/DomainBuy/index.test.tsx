...
it('can present an available domain', () => {
  ...
  expect(screen.getByTestId('domain-buy-text')).toHaveTextContent(['🎉','test.com is available! You can buy it via:'].join(''));
});

it('can present a domain with undefined availability', () => {
  render(<DomainBuy domain="test.com" isAvailable={undefined} />);

  expect(screen.getByTestId('domain-buy-text')).toHaveTextContent(['🤔', 'test.com could be available. Check it using:'].join(''));
});
